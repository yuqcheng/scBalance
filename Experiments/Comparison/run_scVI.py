...
def run_scVI(DataPath, LabelsPath, CV_RDataPath, OutputDir, GeneOrderPath="", NumGenes=0):
    ...
    outputs lists of true and predicted cell labels as csv files, as well as computation time.python
    ...
    if (NumGenes == 0):
        ...
        train = CsvDataset('Dat_ascvi.csv', save_path="", sep=",", labels_file="Labels_scvi.csv", gene_by_cell=False)
    ...
    for i in range(np.squeeze(nfolds)):
        ...
        if (NumGenes > 0):
            ...
            train = CsvDataset('Data_scvi.csv', save_path="", sep=",", labels_file="Labels_scvi.csv",
                               gene_by_cell=False, new_n_genes=False)
...
if __name__=="__main__":
    CV_RDataPath=r'E:\dataset\Single cell classification datasets\Intra-dataset\Pancreatic_data\Baron ' \
                 r'Mouse\CV_folds.RData '
    DataPath=r'E:\dataset\Single cell classification datasets\Intra-dataset\Pancreatic_data\Baron ' \
             r'Mouse\Filtered_MousePancreas_data.csv '
    LabelsPath=r'E:\dataset\Single cell classification datasets\Intra-dataset\Pancreatic_data\Baron ' \
               r'Mouse\Labels.csv'
    OutputDir=r'E:\dataset\Single cell classification datasets\Intra-dataset\Pancreatic_data\Baron Mouse'
    run_scVI(DataPath,LabelsPath,CV_RDataPath,OutputDir)